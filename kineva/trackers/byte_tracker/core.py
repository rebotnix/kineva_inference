...
from kineva.trackers.byte_tracker import matching
from kineva.trackers.byte_tracker.kalman_filter import KalmanFilter
from kineva.trackers.byte_tracker.single_object_track import STrack, TrackState
...
class BYTE_TRACKER:
    ...
    def update_with_detections(self, boxes, confidences, labels, tracker_id):
        """
        ...
        detections = Detections(xyxy=boxes.cpu().numpy(), confidence=confidences.cpu().numpy(), class_id=labels.cpu().numpy(), tracker_id=tracker_id)
        tensors = np.hstack(
            (
                detections.xyxy,
